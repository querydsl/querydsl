...
public class PEntity<D> extends EEntity<D> implements Path<D>{
    ...
    protected <A> PEntity<A> _entity(String path, String entityName, Class<A> type){
        return new PEntity<A>(type, entityName, PathMetadata.forProperty(this, path)); 
    }
    protected <A> PEntityCollection<A> _entitycol(String path, Class<A> type, String entityName) {
        return new PEntityCollection<A>(type, entityName, PathMetadata.forProperty(this, path));
    }
    protected <A> PEntityList<A> _entitylist(String path, Class<A> type, String entityName) {
        return new PEntityList<A>(type, entityName, PathMetadata.forProperty(this,  path));
    }
    protected <K,V> PEntityMap<K,V> _entitymap(String path, Class<K> key, Class<V> value, String entityName){
        return new PEntityMap<K,V>(key, value, entityName, PathMetadata.forProperty(this, path));
    }
    ...
    protected <A> PComponentCollection<A> _simplecol(String path,Class<A> type) {
        return new PComponentCollection<A>(type, PathMetadata.forProperty(this, path));
    }  
    ...
    protected <K,V> PComponentMap<K,V> _simplemap(String path, Class<K> key, Class<V> value){
        return new PComponentMap<K,V>(key, value, PathMetadata.forProperty(this, path));
    }
    protected PString _string(String path){
        return new PString(PathMetadata.forProperty(this, path));
    }
    public AEntity<D> as(PEntity<D> to) {return Grammar.as(this, to);}
