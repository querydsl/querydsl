...
public class DateTimeOperation<T extends Comparable<?>> extends DateTimeExpression<T> implements Operation<T> {
    ...
    public static <D extends Comparable<?>> DateTimeExpression<D> create(Class<D> type, Operator op, Expression<?> one) {
        return new DateTimeOperation<D>(type, op, ImmutableList.<Expression<?>>of(one));
    }
    
    public static <D extends Comparable<?>> DateTimeExpression<D> create(Class<D> type, Operator op, Expression<?> one, Expression<?> two) {
        return new DateTimeOperation<D>(type, op, ImmutableList.of(one, two));
    }
    
    public static <D extends Comparable<?>> DateTimeExpression<D> create(Class<D> type, Operator op, Expression<?>... args) {
        return new DateTimeOperation<D>(type, op, args);
    }

    private final OperationImpl<T> opMixin;

    protected DateTimeOperation(Class<T> type, Operator op, Expression<?>... args) {
        this(type, op, ImmutableList.copyOf(args));
    }

    protected DateTimeOperation(Class<T> type, Operator op, ImmutableList<Expression<?>> args) {
