...
public final class Connections {
    ...
    private static boolean derbyInited, sqlServerInited, h2Inited, hsqlInited, mysqlInited, cubridInited,
...
//        return DriverManager.getConnection(url, "querydsl", "querydsl");
    ...
    public static void initNuoDB()  throws SQLException, ClassNotFoundException{
        targetHolder.set(Target.NUODB);
        Connection c = getNuoDB();
        connHolder.set(c);
        Statement stmt = c.createStatement();
        stmtHolder.set(stmt);
        if (nuodbInited) {
        ...
        stmt.execute("drop schema QUERYDSL cascade");
        stmt.execute("use QUERYDSL");
        // survey
        stmt.execute("create sequence SEQ_T1_F1 start with 1");
        stmt.execute("create table SURVEY(" +
                "ID integer generated by default as identity(SEQ_T1_F1) not null, " +
                "NAME varchar(30)," +
                "NAME2 varchar(30))");
        stmt.execute("insert into SURVEY values (1,'Hello World','Hello');");
        ...
        stmt.execute("create sequence SEQ_EMPLOYEE_ID start with 1");
        stmt.execute("create table EMPLOYEE (" +
                "ID integer generated by default as identity(SEQ_EMPLOYEE_ID) not null, " +
                "FIRSTNAME varchar(50), " +
                "LASTNAME varchar(50), " +
                "SALARY decimal(10,0), " +
                "DATEFIELD date, " +
                "TIMEFIELD time, " +
                "SUPERIOR_ID integer)");
        stmt.execute("alter table EMPLOYEE add primary key (ID)");
        stmt.execute("create index IDX_EMPLOYEE_FK_SUPERIOR on employee (SUPERIOR_ID)");
        stmt.execute("alter table EMPLOYEE add foreign key (SUPERIOR_ID) references EMPLOYEE (ID)");
    ...
    private static Connection getNuoDB() throws SQLException, ClassNotFoundException{
        Class.forName("com.nuodb.jdbc.Driver");
        String url = "jdbc:com.nuodb://localhost/test";
        return DriverManager.getConnection(url, "dba", "goalie");
    }
