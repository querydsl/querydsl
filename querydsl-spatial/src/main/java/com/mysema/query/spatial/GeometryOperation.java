...
public class GeometryOperation<T> extends GeometryExpression<T> implements Operation<T> {

    private static final long serialVersionUID = 3433471874808633698L;

    public static <D> GeometryOperation<D> create(Class<D> type, Operator<? super D> op, Expression<?> one) {
        return new GeometryOperation<D>(type, op, ImmutableList.<Expression<?>>of(one));
    }

    public static <D> GeometryOperation<D> create(Class<D> type, Operator<? super D> op, Expression<?> one, Expression<?> two) {
        return new GeometryOperation<D>(type, op, ImmutableList.of(one, two));
    }

    public static <D> GeometryOperation<D> create(Class<D> type, Operator<? super D> op, Expression<?>... args) {
