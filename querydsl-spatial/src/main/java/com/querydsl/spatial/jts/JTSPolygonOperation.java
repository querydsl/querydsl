...
public class JTSPolygonOperation<T extends Polygon> extends JTSPolygonExpression<T> implements Operation<T> {
    ...
    public static <D extends Polygon> JTSPolygonOperation<D> create(Class<D> type, Operator<? super D> op, Expression<?> one) {
        return new JTSPolygonOperation<D>(type, op, ImmutableList.<Expression<?>>of(one));
    }

    public static <D extends Polygon> JTSPolygonOperation<D> create(Class<D> type, Operator<? super D> op, Expression<?> one, Expression<?> two) {
        return new JTSPolygonOperation<D>(type, op, ImmutableList.of(one, two));
    }

    public static <D extends Polygon> JTSPolygonOperation<D> create(Class<D> type, Operator<? super D> op, Expression<?>... args) {
        return new JTSPolygonOperation<D>(type, op, args);
    }

    private final OperationImpl< T> opMixin;

    protected JTSPolygonOperation(Class<T> type, Operator<? super T> op, Expression<?>... args) {
        this(type, op, ImmutableList.copyOf(args));
    }

    protected JTSPolygonOperation(Class<T> type, Operator<? super T> op, ImmutableList<Expression<?>> args) {
