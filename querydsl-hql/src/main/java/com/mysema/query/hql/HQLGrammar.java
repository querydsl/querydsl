...
public class HQLGrammar extends GrammarWithAlias {
    ...
    public static <D extends Number & Comparable<? super D>> ENumber<D> all(
            CollectionType<D> col) {
    ...
    public static EComparable<Date> hour(Expr<Date> date) {
        return operationFactory.createComparable(Date.class,
                Ops.OpDateTime.HOUR, date);
    }

    public static PComponentCollection<Integer> indices(PCollection<?> col) {
        return new PComponentCollection<Integer>(Integer.class,
                new PathMetadata<Collection<Integer>>(col, null,
                        HqlPathType.LISTINDICES));
    }

    public static <K, V> PComponentCollection<K> indices(PMap<K, V> col) {
        return new PComponentCollection<K>(col.getKeyType(),
                new PathMetadata<Collection<Integer>>(col, null,
                        HqlPathType.LISTINDICES));
    }
    ...
    public static <A extends Comparable<? super A>> EComparable<A> max(
            PCollection<A> left) {
        return new QComparable<A>(left.getElementType(), OpQuant.MAX_IN_COL,
                left);
    }

    public static <A> PEntity<A> maxelement(PEntityCollection<A> col) {
        return new PEntity<A>(col.getElementType(), col.getEntityName(),
                new PathMetadata<A>(col, null, HqlPathType.MINELEMENT));
    }

    public static <A> PComparable<Integer> maxindex(PComponentCollection<A> col) {
        return new PComparable<Integer>(Integer.class,
                new PathMetadata<Integer>(col, null, HqlPathType.MAXINDEX));
    }

    public static <A> PNumber<Integer> maxindex(PEntityCollection<A> col) {
        return new PNumber<Integer>(Integer.class, new PathMetadata<Integer>(
                col, null, HqlPathType.MAXINDEX));
    }

    public static <A extends Comparable<? super A>> EComparable<A> min(
            PCollection<A> left) {
        return new QComparable<A>(left.getElementType(), OpQuant.MIN_IN_COL,
                left);
    }
    ...
    public static <A> PComparable<Integer> minindex(PComponentCollection<A> col) {
        return new PComparable<Integer>(Integer.class,
                new PathMetadata<Integer>(col, null, HqlPathType.MININDEX));
    }
    ...
    public static EComparable<Date> minute(Expr<Date> date) {
        return operationFactory.createComparable(Date.class,
                Ops.OpDateTime.MINUTE, date);
    }

    public static EComparable<Date> month(Expr<Date> date) {
        return operationFactory.createComparable(Date.class,
                Ops.OpDateTime.MONTH, date);
    }
    ...
    public static <D extends Number & Comparable<? super D>> ENumber<?> sum(
            Expr<D> left) {
        Class<?> type = left.getType();
        if (type.equals(Byte.class) || type.equals(Integer.class)
                || type.equals(Short.class))
            type = Long.class;
        if (type.equals(Float.class))
            type = Double.class;
        return operationFactory.createNumber((Class<D>) type, HQLOps.OpHql.SUM,
                left);
    }

    public static <D extends Number & Comparable<? super D>> ENumber<Long> sumAsLong(
            Expr<D> left) {
        return sum(left).longValue();
    }

    public static <D extends Number & Comparable<? super D>> ENumber<Double> sumAsDouble(
            Expr<D> left) {
