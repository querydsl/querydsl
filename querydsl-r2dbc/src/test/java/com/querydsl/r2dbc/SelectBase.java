...
public abstract class SelectBase extends AbstractBaseTest {
    ...
    private final ReactiveQueryExecution standardTest = new ReactiveQueryExecution(Module.SQL, Connections.getTarget()) {
        @Override
    ...
    public void aggregate_list() {
        ...
        // fetch
        assertEquals(min, query().from(employee).select(employee.salary.min()).fetch().collectList().block().get(0).intValue());
        assertEquals(avg, query().from(employee).select(employee.salary.avg()).fetch().collectList().block().get(0).intValue());
        assertEquals(max, query().from(employee).select(employee.salary.max()).fetch().collectList().block().get(0).intValue());
    }
    ...
    public void aggregate_uniqueResult() {
        int min = 30000, avg = 65000, max = 160000;
        // fetchOne
        assertEquals(min, query().from(employee).select(employee.salary.min()).fetchOne().block().intValue());
        assertEquals(avg, query().from(employee).select(employee.salary.avg()).fetchOne().block().intValue());
        assertEquals(max, query().from(employee).select(employee.salary.max()).fetchOne().block().intValue());
    }
    ...
    public void alias_quotes() {
        ...
        query().from(employee).select(employee.firstname.as("First Name")).fetch().collectList().block();
    }
    ...
    public void array_projection() {
        List<String[]> results = query().from(employee).select(
                new ArrayConstructorExpression<String>(String[].class, employee.firstname)).fetch().collectList().block();
    ...
    @Test
    public void beans() {
        List<Beans> rows = query().from(employee, employee2).select(new QBeans(employee, employee2)).fetch().collectList().block();
    ...
    public void case_() {
        ...
        assertEquals(Arrays.asList(87, 90, 88, 87, 83, 80, 75),
                query().from(employee, employee2)
                        ...
                        .select(numExpression2.floor().intValue()).fetch().collectList().block());
    }
    ...
    public void coalesce() {
        ...
        assertEquals(Arrays.asList(),
                query().from(employee).where(c.getValue().eq("xxx")).select(employee.id).fetch().collectList().block());
    }
    ...
    public void complex_boolean() {
        ...
        assertEquals(0, (long) query().from(employee).where(
    ...
    public void complex_subQuery() {
        ...
        ).select(sq.get(sal).avg(), sq.get(sal).min(), sq.get(sal).max()).fetch().collectList().block();
    }

    @Test
    public void constructor_projection() {
        for (IdName idAndName : query().from(survey).select(new QIdName(survey.id, survey.name)).fetch().collectList().block()) {
    ...
    public void constructor_projection2() {
        List<SimpleProjection> projections = query().from(employee).select(
                Projections.constructor(SimpleProjection.class,
                        employee.firstname, employee.lastname)).fetch().collectList().block();
    ...
    @Test
    public void count2() {
        assertEquals(10, query().from(employee).select(employee.count()).fetchFirst().block().intValue());
    }
    ...
    public void count_all() {
        ...
        assertEquals(10, query().from(employee).select(Wildcard.count.as(rowCount)).fetchOne().block().intValue());
    }
    ...
    public void custom_projection() {
        List<Projection> tuples = query().from(employee).select(
                new QProjection(employee.firstname, employee.lastname)).fetch().collectList().block();
...
//        data.add(javaInstant);                                      //java.time.Instant
    ...
    public void date_diff() {
        ...
        R2DBCQuery<?> query2 = query().from(employee, employee2)
    ...
    public void date_trunc2() {
        DateTimeExpression<LocalDateTime> expr = DateTimeExpression.currentTimestamp(LocalDateTime.class);
    ...
    public void dateTime() {
        R2DBCQuery<?> query = query().from(employee).orderBy(employee.id.asc());
        assertEquals(Integer.valueOf(10), query.select(employee.datefield.dayOfMonth()).fetchFirst().block());
        assertEquals(Integer.valueOf(2), query.select(employee.datefield.month()).fetchFirst().block());
        assertEquals(Integer.valueOf(2000), query.select(employee.datefield.year()).fetchFirst().block());
        assertEquals(Integer.valueOf(200002), query.select(employee.datefield.yearMonth()).fetchFirst().block());
    }
    ...
    public void distinct_list() {
        List<Integer> lengths1 = query().from(employee).distinct().select(employee.firstname.length()).fetch().collectList().block();
        List<Integer> lengths2 = query().from(employee).distinct().select(employee.firstname.length()).fetch().collectList().block();
    ...
    public void duplicate_columns() {
        assertEquals(10, query().from(employee)
                .select(employee.id, employee.id).fetch().collectList().block().size());
    }

    @Test
    public void duplicate_columns_In_Subquery() {
        QEmployee employee2 = new QEmployee("e2");
        assertEquals(10, (long) query().from(employee).where(
                query().from(employee2)
                        .where(employee2.id.eq(employee.id))
                        .select(employee2.id, employee2.id).exists()).fetchCount().block());
    }
    ...
    public void factoryExpression_in_groupBy() {
        ...
        assertTrue(query().from(employee).groupBy(empBean).select(empBean).fetchFirst().block() != null);
    }
...
//    }
    ...
    public void groupBy_yearMonth() {
        assertEquals(Arrays.asList(10L), query().from(employee)
                ...
                .select(employee.id.count()).fetch().collectList().block());
    }
    ...
    public void groupBy_validate() {
        ...
        assertEquals(8, query().from(employee)
                ...
                .select(employee.salary.multiply(100).as(alias),
                        employee.salary.avg()).fetch().collectList().block().size());
    }
    ...
    public void in() {
        assertEquals(2, query().from(employee)
                ...
                .select(employee).fetch().collectList().block().size());
    }
    ...
    public void notIn_long_list() {
        ...
        assertEquals(0, (long) query().from(employee).where(employee.id.notIn(ids)).fetchCount().block());
    }

    @Test
    public void in_empty() {
        assertEquals(0, (long) query().from(employee).where(employee.id.in(Collections.emptyList())).fetchCount().block());
    }

    @Test
    @ExcludeIn(DERBY)
    public void in_null() {
        assertEquals(1, (long) query().from(employee).where(employee.id.in(1, null)).fetchCount().block());
    }
    ...
    public void in_subqueries() {
        ...
        assertEquals(2, (long) query().from(employee).where(employee.id.in(
    ...
    public void notIn_empty() {
        ...
        assertEquals(count, (long) query().from(employee).where(employee.id.notIn(Collections.emptyList())).fetchCount().block());
    }
    ...
    public void join() throws Exception {
        for (String name : query().from(survey, survey2)
                .where(survey.id.eq(survey2.id)).select(survey.name).fetch().collectList().block()) {
            assertNotNull(name);
        }
    }
    ...
    public void joins() {
        for (Tuple row : query().from(employee).innerJoin(employee2)
                ...
                .select(employee.id, employee2.id).fetch().collectList().block()) {
    ...
    @Test
    public void like() {
        assertEquals(0, (long) query().from(employee).where(employee.firstname.like("\\")).fetchCount().block());
        assertEquals(0, (long) query().from(employee).where(employee.firstname.like("\\\\")).fetchCount().block());
    }

    @Test
    public void like_ignore_case() {
        assertEquals(3, (long) query().from(employee).where(employee.firstname.likeIgnoreCase("%m%")).fetchCount().block());
    }
        ...
        for (String str : strs) {
            assertTrue(str, query()
                    ...
                    .where(Expressions.predicate(Ops.STRING_CONTAINS,
                            ...
                            Expressions.constant(str))).fetchCount().block() > 0);
        }
    }
    ...
    public void like_number() {
        assertEquals(5, (long) query().from(employee)
                .where(employee.id.like("1%")).fetchCount().block());
    }
    ...
    public void limit() {
        assertEquals(Arrays.asList(23, 22, 21, 20), query().from(employee)
                ...
                .limit(4).select(employee.id).fetch().collectList().block());
    }
...
//    }
    ...
    public void limit_and_offset_and_Order() {
        ...
        assertEquals(names2, query().from(employee)
                .orderBy(employee.firstname.asc())
                .limit(4).offset(2)
                .select(employee.firstname).fetch().collectList().block());
    }
...
//    }
    ...
    public void math3() {
        ...
        Double num = query().select(one.add(two.multiply(three)).subtract(four.divide(five)).add(six.mod(three))).fetchFirst().block();
    ...
    public void nested_tuple_projection() {
        ...
        List<Tuple> tuples = query().from(employee)
                .select(employee.firstname, employee.lastname, concat).fetch().collectList().block();
        assertFalse(tuples.isEmpty());
    ...
    @Test
    public void nullif() {
        query().from(employee).select(employee.firstname.nullif(employee.lastname)).fetch().collectList().block();
    }
    ...
    public void num_cast() {
        ...
        query().from(employee).select(employee.id.castToNum(Double.class)).fetch().collectList().block();
    }
    ...
    public void operation_in_constant_list() {
        assertEquals(0, (long) query().from(survey).where(survey.name.charAt(0).in(Arrays.asList('a'))).fetchCount().block());
        assertEquals(0, (long) query().from(survey).where(survey.name.charAt(0).in(Arrays.asList('a', 'b'))).fetchCount().block());
        assertEquals(0, (long) query().from(survey).where(survey.name.charAt(0).in(Arrays.asList('a', 'b', 'c'))).fetchCount().block());
    }
    ...
    public void order_nullsFirst() {
        assertEquals(Arrays.asList("Hello World"), query().from(survey)
                ...
                .select(survey.name).fetch().collectList().block());
    }
    ...
    public void params() {
        ...
        assertEquals("Mike", query()
                ...
                .set(name, "Mike")
                .select(employee.firstname).fetchFirst().block());
    }
    ...
    public void path_alias() {
        ...
        query().from(employee)
                ...
                .select(employee.lastname, salarySum).fetch().collectList().block();
    }

    @Test
    public void path_in_constant_list() {
        assertEquals(0, (long) query().from(survey).where(survey.name.in(Arrays.asList("a"))).fetchCount().block());
        assertEquals(0, (long) query().from(survey).where(survey.name.in(Arrays.asList("a", "b"))).fetchCount().block());
        assertEquals(0, (long) query().from(survey).where(survey.name.in(Arrays.asList("a", "b", "c"))).fetchCount().block());
    }
    ...
    public void projection_and_twoColumns() {
        ...
        for (Tuple row : query().from(survey)
                .select(new QIdName(survey.id, survey.name), survey.id, survey.name).fetch().collectList().block()) {
    ...
    public void qBeanUsage() {
        ...
        List<Survey> surveys =
                query().from(
                        ...
                        .select(Projections.bean(Survey.class, Collections.singletonMap("name", sq.get(survey.name)))).fetch().collectList().block();
    ...
    public void query_with_constant() throws Exception {
        for (Tuple row : query().from(survey)
                ...
                .select(survey.id, survey.name).fetch().collectList().block()) {
    ...
    public void query2() throws Exception {
        for (Tuple row : query().from(survey).select(survey.id, survey.name).fetch().collectList().block()) {
            assertNotNull(row.get(survey.id));
            assertNotNull(row.get(survey.name));
        }
    }
    ...
    public void relationalPath_projection() {
        List<Tuple> results = query().from(employee, employee2).where(employee.id.eq(employee2.id))
                .select(employee, employee2).fetch().collectList().block();
        assertFalse(results.isEmpty());
    ...
    public void relationalPath_eq2() {
        assertEquals(1, query().from(survey, survey2)
                ...
                .select(survey.id, survey2.id).fetch().collectList().block().size());
    }
    ...
    public void select_concat() {
        for (Tuple row : query().from(survey).select(survey.name, survey.name.append("Hello World")).fetch().collectList().block()) {
            assertEquals(
    ...
    public void select_for_update() {
        assertEquals(1, query().from(survey).forUpdate().select(survey.id).fetch().collectList().block().size());
    }

    @Test
    @ExcludeIn({SQLITE, CUBRID, TERADATA})
    public void select_for_update_Where() {
        assertEquals(1, query().from(survey).forUpdate().where(survey.id.isNotNull()).select(survey.id).fetch().collectList().block().size());
    }
    ...
    public void select_for_share() {
        if (configuration.getTemplates().isForShareSupported()) {
            assertEquals(1, query().from(survey).forShare().where(survey.id.isNotNull()).select(survey.id).fetch().collectList().block().size());
        } else {
            try {
                query().from(survey).forShare().where(survey.id.isNotNull()).select(survey.id).fetch().collectList().block().size();
                fail();
    ...
    public void serialization2() throws Exception {
        List<Tuple> rows = query().from(survey).select(survey.id, survey.name).fetch().collectList().block();
        serialize(rows);
    }
    ...
    public void single_array() {
        assertNotNull(query().from(survey).select(new Expression<?>[]{survey.name}).fetchFirst().block());
    }
    ...
    public void single_column_via_Object_type() {
        for (Object s : query().from(survey)
                .select(ExpressionUtils.path(Object.class, survey.name.getMetadata())).fetch().collectList().block()) {
    ...
    public void specialChars() {
        assertEquals(0, (long) query().from(survey).where(survey.name.in("\n", "\r", "\\", "\'", "\"")).fetchCount().block());
    }
    ...
    public void syntax_for_employee() {
        assertEquals(3, query().from(employee).groupBy(employee.superiorId)
                ...
                .select(employee.salary.avg(), employee.id.max()).fetch().collectList().block().size());
...
//    }
    ...
    public void tuple_projection() {
        List<Tuple> tuples = query().from(employee)
                .select(employee.firstname, employee.lastname).fetch().collectList().block();
        assertFalse(tuples.isEmpty());
        for (Tuple tuple : tuples) {
            assertNotNull(tuple.get(employee.firstname));
            assertNotNull(tuple.get(employee.lastname));
    ...
    public void tuple2() {
        assertEquals(10, query().from(employee)
                .select(Expressions.as(ConstantImpl.create("1"), "code"),
                        employee.id).fetch().collectList().block().size());
    }
    ...
    public void twoColumns_and_projection() {
        ...
        for (Tuple row : query().from(survey)
                .select(survey.id, survey.name, new QIdName(survey.id, survey.name)).fetch().collectList().block()) {
            assertEquals(3, row.size());
            assertEquals(Integer.class, row.get(0, Object.class).getClass());
            assertEquals(String.class, row.get(1, Object.class).getClass());
    ...
    public void unique_Constructor_projection() {
        IdName idAndName = query().from(survey).limit(1).select(new QIdName(survey.id, survey.name)).fetchFirst().block();
        assertNotNull(idAndName);
        assertNotNull(idAndName.getId());
        assertNotNull(idAndName.getName());
    }
    ...
    @Test
    public void various() {
        for (String s : query().from(survey).select(survey.name.lower()).fetch().collectList().block()) {
            assertEquals(s, s.toLowerCase());
        }

        for (String s : query().from(survey).select(survey.name.append("abc")).fetch().collectList().block()) {
    ...
    public void with2() {
        ...
        assertEquals(100, query().with(employee2, query().from(employee)
                ...
                .select(employee.id, employee2.id, employee3.id).fetch().collectList().block().size());
    }
    ...
    public void wildcard_all2() {
        assertEquals(10, query().from(new RelationalPathBase<Object>(Object.class, "employee", "public", "EMPLOYEE"))
                .select(Wildcard.all).fetch().collectList().block().size());
    }
    ...
    public void withinGroup() {
        ...
        for (WithinGroup<?> wg : exprs) {
            ...
            query().from(survey).select(wg.withinGroup().orderBy(survey.id.asc(), survey.id.asc())).fetch().collectList().block();
        }
        ...
        for (WithinGroup<?> wg : exprs) {
            ...
            query().from(survey).select(wg.withinGroup().orderBy(survey.id.asc())).fetch().collectList().block();
        }
    }
