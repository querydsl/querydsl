    ...
    public String serialize(String literal, int jdbcType) {
        switch (jdbcType) {
            ...
            case TIMESTAMP_WITH_TIMEZONE:
                try {
                    return String.valueOf(dateTimeFormatter.parse(literal).getTime());
                } catch (ParseException e) {
                    throw new RuntimeException(e);
                }
            case Types.DATE:
                try {
                    return String.valueOf(dateFormatter.parse(literal).getTime());
                } catch (ParseException e) {
                    throw new RuntimeException(e);
                }
            case Types.TIME:
            case TIME_WITH_TIMEZONE:
                try {
                    return String.valueOf(timeFormatter.parse(literal).getTime());
                } catch (ParseException e) {
                    throw new RuntimeException(e);
                }
            default:
